            ...
            <tbody>
                @forelse ($jenisKendaraans as $jk)
                    <tr>
                        ...
                        <td>{{ $jk->deskripsi }}</td>
                        <td>
                            <a href="{{ route('jenis_kendaraan.edit', $jk->id_jenis) }}" class="btn btn-warning btn-sm">Edit</a>
                            <form action="{{ route('jenis_kendaraan.destroy', $jk->id_jenis) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
